...
class LuminaSender_LuminaRobot {
    var luminaHelperPrebuild = LuminaHelper_Prebuild()
    var luminaHelperDetectUtil =
        LuminaHelper_Detector()
    var luminaSenderCourier = LuminaSender_Courier()
...
//        Long groupIdBox = event.getGroupId();
        ...
        luminaRequireSetup = luminaHelperPrebuild.setLuminaAtRule(luminaRequireSetup, event.rawMessage)
        ...
        val matchedKeyword = luminaHelperDetectUtil.checkDetectMainKeyword(event.rawMessage, luminaRequireSetup.masterKeyword!!, luminaRequireSetup.secondKeyword, luminaRequireSetup.isRequireKeywordMatch)
        //  测试区 测试完需删除
        val otherKeywordListMatched = luminaHelperDetectUtil.checkDetectOtherKeywordList(event.rawMessage, luminaRequireSetup.otherKeywordList!!)
        ...
        var msg_running: Msg
        if (luminaRequireSetup.isRequireRespReply() && atNotDetect && matchedKeyword && luminaHelperDetectUtil.checkDetectOtherKeywordList(event.rawMessage, luminaRequireSetup.otherKeywordList!!)) {
//            msg_running = Msg.builder()
//                    .reply(event.getMessageId())
//                    .text(listText.get(0));
            bot.sendGroupMsg(event.groupId, luminaSenderCourier.buildBasicReplyMessage(event, listText as MutableList<String>), false)
        } else if (!luminaRequireSetup.isRequireRespReply() && atNotDetect && matchedKeyword && luminaHelperDetectUtil.checkDetectOtherKeywordList(event.rawMessage, luminaRequireSetup.otherKeywordList!!)) {
//            msg_running = Msg.builder()
//                    .at(event.getUserId())
//                    .text(listText.get(0));
            bot.sendGroupMsg(event.groupId, luminaSenderCourier.buildBasicAtUserMessage(event, listText as MutableList<String>), false)
        }
...
//        Long groupIdBox = event.getGroupId();
        ...
        if (luminaRequireSetup.otherKeywordList == null) {
            if (luminaRequireSetup.isRequireRespReply() && atNotDetect && matchedKeyword) {
                bot.sendGroupMsg(event.groupId, luminaSenderCourier.buildMultiReplyPairMessage(event, luminaRequireSetup.hybridRespondMap!!), false)
            } else if (!luminaRequireSetup.isRequireRespReply() && atNotDetect && matchedKeyword && luminaHelperDetectUtil.checkDetectOtherKeywordList(event.rawMessage, luminaRequireSetup.otherKeywordList!!)) {
                bot.sendGroupMsg(event.groupId, luminaSenderCourier.buildMultiAtUserPairMessage(event, luminaRequireSetup.hybridRespondMap!!), false)
            }
        } else {
            if (luminaRequireSetup.isRequireRespReply() && atNotDetect && matchedKeyword && luminaHelperDetectUtil.checkDetectOtherKeywordList(event.rawMessage, luminaRequireSetup.otherKeywordList!!)) {
                bot.sendGroupMsg(event.groupId, luminaSenderCourier.buildMultiReplyPairMessage(event, luminaRequireSetup.hybridRespondMap!!), false)
